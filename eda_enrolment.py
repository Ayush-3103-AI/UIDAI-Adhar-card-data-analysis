"""
Comprehensive Exploratory Data Analysis (EDA) for Aadhaar Enrolment Data
...
output_dir = Path("visualizations/enrolment")
output_dir.mkdir(parents=True, exist_ok=True)

print("=" * 80)
print("COMPREHENSIVE EDA: AADHAAR ENROLMENT DATA")
print("=" * 80)
...
data_folder = Path("api_data_aadhar_enrolment/api_data_aadhar_enrolment")
...
if all(col in df.columns for col in ['age_0_5', 'age_5_17', 'age_18_greater']):
    ...
    total_all = sum(total_by_age.values())
    
    print("\nTotal enrolments by age group:")
    for age_group, count in total_by_age.items():
        pct = (count / total_all) * 100
        print(f"  {age_group:15s}: {count:12,} ({pct:5.2f}%)")
...
if len(numeric_cols) > 1:
    ...
    plt.title('Correlation Heatmap: Enrolment Age Groups', fontsize=16, fontweight='bold', pad=20)
    plt.tight_layout()
...
if all(col in df.columns for col in ['age_0_5', 'age_5_17', 'age_18_greater']):
    print("\nGenerating age group comparison...")
    age_totals = {
        'Age 0-5': df['age_0_5'].sum(),
        'Age 5-17': df['age_5_17'].sum(),
        'Age 18+': df['age_18_greater'].sum()
    ...
    ax1.set_title('Enrolment Distribution by Age Group', fontsize=14, fontweight='bold')
    
    # Bar chart
    ax2.bar(age_totals.keys(), age_totals.values(), color=['#ff9999', '#66b3ff', '#99ff99'])
    ax2.set_title('Total Enrolments by Age Group', fontsize=14, fontweight='bold')
    ...
    plt.close()
    print(f"  [OK] Saved: {output_dir / 'age_group_comparison.png'}")
...
# Save summary to CSV
summary_df.to_csv(output_dir.parent / "enrolment_summary_stats.csv", index=False)
print(f"\n  [OK] Summary statistics saved to: {output_dir.parent / 'enrolment_summary_stats.csv'}")

print("\n" + "=" * 80)
print("EDA COMPLETE: ENROLMENT DATA")
print("=" * 80)
